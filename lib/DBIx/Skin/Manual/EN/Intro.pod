...
DBIx::Skin::Manual::EN::Intro - Intro to DBIx::Skin
...
L<DBIx::Skin> runs on MySQL, SQLite, PostgreSQL and Oracle.
...
Let's work with an example project called C<Proj>. Your Skin 
model for this project would be:

    package Proj::Model;
    use DBIx::Skin conect_info => +{
        dsn => 'dbi:SQLite:',
        username => '',
        password => '',
    };
...
L<DBIx::Skin::Schema::Loader>. However, read on for info about
manual setup.
...
Unlike most OR mappers which require a class for every table, in Skin
...
Skin supports insert/update/delete triggers:
...
C<< ->insert() >> returns a L<DBIx::Skin::Row>> class, so you can access
columns like so:
...
Please see L<DBIx::Skin::Manual::EN::Resultset>

=head2 Transactions

Transactions are easy in Skin:
