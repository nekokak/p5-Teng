...
DBIx::Skin::Manual::JA::Intro - DBIx::Skinの日本語ドキュメント

=head1 DESCRIPTION

Skinの総合的な使い方を網羅するマニュアルです。

Please translate and read the person in the sphere in English. 

=head1 注意

現在SkinはMySQLとSQLiteとPostgreSQLとOracleをサポートしています。

他のDBDを使いたい場合は、
DBIx::Skin::DBD::*を作っていただく必要があります。

まだまだα版なので色々変わる事があるかもしれません。

=head2 Skinの基本クラスの定義

Skinを操作するClassを定義します。

例えばProjというプロジェクトでSkinを使う場合
...
DBIx::Skinをuseする時の引数でdsnなどDBの接続に必要な情報を渡す事ができます。
...
DBIx::Skinをuseする時の引数にdsnなどを書かずにClassを作成しておき、
DBにクエリを投げる前に
...
Skinでは他のORマッパーと同じように各tableに対応するschemaの設定を書く必要があります。
...
DBIx::Skin::SchemaをuseするとSchemaを構成するために必要なmethodがexportされます。

ちなみに、DBIx::Skin::Schemaでもuseすると
...
Skinでは他のORマッパーと異なり、テーブル毎にClassを作る必要はありません。
...
ここで注意が必要なのですが、Skinはルールベースな設計しています。

Skinの根本思想としてはモジュール利用者が好きに生のSQLを実行して、
その結果をちょっといい感じのオブジェクトに纏めて
ちょっと楽をしようというものです。

脱線しましたがinstall_utf8_columnsに対象となるカラム名を設定するのですが
この設定はSkinで取り扱う全テーブルに対しての設定となります。
...
Skinにもinflate/deflateの処理を書く事ができます
...
install_inflate_ruleもinstall_utf8_columnsと同様Skinで扱う全テーブルが対象となります。

=head3 triggerについて

Skinにもinsert/update/deleteなどを行った場合にtriggerによるHookをかける事ができます。
...
Skinはインスタンスを作っても作らなくてもDBを操作する事もできるようになっています。
...
既にdatabase handlerを別で管理しており、Skinでそのhandlerを使いたい場合は
set_dbhメソッドを使用します。
...
insertメソッドの返り値はSkinのRowクラスになっていますので
...
返り値の情報が変わりま。
スカラーコンテキストで受けた場合はDBIx::Skin::Iteratorが取得でき、
...
DBIx::Skin::Manual::JA::Resultsetを参照してください。

=head2 トランザクション

Skinではトランザクションの仕組みを簡単にサポートしています。
...
Skinのトランザクションサポートはtxn_scopeメソッドで取得したオブジェクトが有効な間、
トランザクションの面倒をみます。
