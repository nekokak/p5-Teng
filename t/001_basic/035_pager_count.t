...
subtest 'simple_with_group_by_error' => sub {
    local $@;
    eval {
        my ($rows, $pager) = $db->search_with_pager(mock_basic => {}, {group_by => ['name'], rows => 3, page => 1});
    };
    like $@, qr{Cannot use group_by option with Teng::Plugin::Pager::Count::search_with_pager};
};

subtest 'simple_with_group_by_alternative_pager' => sub {
    Mock::Basic->load_plugin('Pager::Count', {alternative_pager => 'Pager'});
